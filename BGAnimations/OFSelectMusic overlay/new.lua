...
local function MoveSong(self, offset, Songs, reset)
	if reset then
		...
		while Index.Song > #Songs do Index.Song = Index.Song - #Songs end
		while Index.Song < 1 do Index.Song = #Songs + Index.Song end
...
local function MoveGroup(self, offset, Groups)
	if reset then
		Index.Group = 1
		wheel.Group.Dec = 6
		wheel.Group.Inc = -6
...
groupWheel
	...
	:SetCommand('Init', function(self)
		self
			...
			:SetFastCatchup(true)
			:aux(0)
		MoveGroup(self, 0, AllGroups, true)
	end)
