...
menu_items[#menu_items + 1] = {
	...
	item_type = "menu",
	get = function()
		local ProfileImage = LoadModule("Config.Load.lua")("AvatarImage",
			"/Save/LocalProfiles/" .. profile_id .. "/OutFoxPrefs.ini")
...
for i, item in ipairs(menu_items) do
	...
	local menuitemnew
	if item.get then
		local value_text = type(item.get) == "string" and item_value_to_text(item, profile[item.get](profile)) or
		item.get()
		-- 4.  The Menu Values
	...
	args[#args + 1] = Def.ActorFrame {
		...
		Def.ActorFrame {
			...
			Def.Quad { InitCommand = function(self)
				self:x(0)
					:faderight(0.5)
					:zoomto(SCREEN_WIDTH, 4):halign(0):vertalign(top):y(-52 / 2):diffuse(PlayerColor(master))
					:diffuseleftedge(ColorLightTone(PlayerColor(master)))
			end
			},
			Def.Quad { InitCommand = function(self)
				self:x(0)
					:faderight(0.5)
					:zoomto(SCREEN_WIDTH, 4):halign(0):vertalign(bottom):y(52 / 2):diffuse(ColorLightTone(PlayerColor(
				master))):diffuseleftedge(PlayerColor(master))
			end
			},
		},
...
args[#args + 1] = Def.ActorFrame {
	...
	Def.BitmapText { Font = "Common Normal", Text = profile:GetDisplayName(), OnCommand = function(s) s:xy(50 + 96, 100)
			:zoom(1.2):halign(0) end, },
	Def.BitmapText { Font = "Common Normal", Text = "GUID: " .. profile:GetGUID(), OnCommand = function(s) s:xy(50 + 96,
			100 + 4 + (28 * 1)):halign(0) end, },
	Def.BitmapText { Font = "Common Normal", Text = Screen.String("TotalTime") ..
	": " .. SecondsToHHMMSS(profile:GetTotalGameplaySeconds()), OnCommand = function(s) s:xy(50 + 96, 100 + 4 + (28 * 2))
			:halign(0) end, },
}
